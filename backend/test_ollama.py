...
async def test_manual_integration():
    ...
    if not await llm_service.check_health():
        print("Ollama is not running. Please start Ollama to run this test.")
        return
        
    if not await llm_service.check_model():
        print(f"Model '{llm_service.model_name}' is not installed in Ollama.")
        print(f"Please run: ollama pull {llm_service.model_name}")
        return
        
    print("Ollama is ready. Warming up model...")
